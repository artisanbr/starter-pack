...
<div>
    ...
    <!-- HEADER -->
    <x-starter-pack::layouts.dashboard.toolbar title="Lista de Grupos"
                                               subtitle="Gerenciar Grupos de Usuários">
        <x-slot:actions>
            {!! $this->renderActions() !!}
        </x-slot:actions>
    ...
    <x-modal wire:model="showFormModal"
             :title="$form->id ? ($readonly ? 'Visualizando' : 'Editando').' '.$form->display_name : 'Novo Grupo'"
             class="backdrop-blur">
                ...
                @if($readonly)
                    ...
                    <div class="flex flex-wrap gap-3 ">
                        @foreach ($this->modelPermissions() as $permission)
                            <x-badge :value="$permission->display_name" class="w-auto whitespace-nowrap badge-primary"/>
                        @endforeach
                    </div>
                @else
                    <x-choices label="Permissões Concedidas" wire:model="form.permissions"
                               :options="$permissions"
